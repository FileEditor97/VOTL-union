...
public class LevelRolesManager extends LiteDBBase {
	...
	public boolean add(long guildId, int level, String roleIds, boolean exact, ExpType type) {
		...
		return execute("INSERT INTO %s(guildId, level, roles, exact, type) VALUES (%d, %d, %s, %d, %d) ON COFLICT(guildId, level) DO UPDATE SET roles=%4$s, exact=%5$d, type=%6$d".formatted(table, guildId, level, quote(roleIds), exact?1:0, type.value));
	}
